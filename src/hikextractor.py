...
def parse_master_block(mappedfile) -> MasterBlock:
    ...
    return MasterBlock(signature=signature, capacity=capacity,
                       version=version,
...
def parse_hbt_entry(data, offset) -> HIKBTREEEntry:
    ...
    return HIKBTREEEntry(channel=channel,
...
def parse_idr_header_page(page: bytes, offset_datablock):
    ...
    return IDRHeaderPage(channel=channel,
                         start_timestamp=start_timestamp,
                         end_timestamp=end_timestamp,
...
def parse_idr_header(datablock, masterblock: MasterBlock):
    ...
    return IDRHeader(channel=channel,
...
def export_footage_from_block(datablock, outfile):
    ...
    if start_offset < 0:
        return
    while True:
        end_offset = find_in_bytes(
            datablock, BA_NAL, start_offset + 5, 120*1024)
        if end_offset < 0:
            return
...
def export_file(datablock, filename):
    with subprocess.Popen(['ffmpeg', '-i', '-', '-c:v', 'copy',
                           '-bsf:v', 'filter_units=pass_types=1-5',
                           '-aspect', '4/3', '-loglevel', 'error',
                           '-stats', filename],
                          stdin=subprocess.PIPE) as ffmpeg:
    ...
    with open(source, 'rb') as input_image, \
        ...
        try:
            ...
            if master.version != b"HIK.2011.03.08":
                print("This script was tested only with version HIK.2011.03.08"
        ...
        for entry in entrylist:
            if entry.recording:
                ...
                filename = (f'CH-{entry.channel:02d}__'
                            f'{start:%Y-%m-%d-%H-%M}__'
                            f'{end:%Y-%m-%d-%H-%M}.mp4')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', dest='input',
                        required=True, help='Raw image file from the DVR HD')
